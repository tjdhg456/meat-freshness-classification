...
def train(model, optimizer, epoch, train_loader, option, criterion, scheduler=None):
    ...
    for ix, (tr_x, tr_reg, tr_cls) in enumerate(train_loader):
        ...
        if option['type_name'] == 'reg':
            pred_reg, feature = model(tr_x)
            loss_tr = cri_reg(pred_reg.view(-1), tr_reg)

        elif option['type_name'] == 'cls':
            pred_cls, feature = model(tr_x)
            if cri_center is not None:
                loss_center = cri_center(pred_cls, tr_cls)
                loss_tr = cri_cls(pred_cls, tr_cls) + loss_center * option['alpha']
            else:
        ...
        if cri_center is not None:
            for param in cri_center.parameters():
                param.grad.data *= (option['lr_cent'] / (option['alpha'] * option['lr']))
        optimizer.step()
    ...
    if (epoch+1) % option['print_epoch'] == 0:
        print('\n Epoch : %d, Train Acc : %2.2f, Train Loss : %.2f,' % (epoch, acc_tr, loss_))
    return model
...
def test(model, epoch, test_loader, option, criterion):
    ...
    with torch.no_grad():
        for ix, (te_x, te_reg, te_cls) in enumerate(test_loader):
            ...
            # Foward Propagation
            if option['type_name'] == 'reg':
                pred_reg, _ = model(te_x)
                loss_te = cri_reg(pred_reg.view(-1), te_reg)

            elif option['type_name'] == 'cls':
                pred_cls, _ = model(te_x)
...
def ad_train(model, aux_data, optimizer, epoch, train_loader, option, criterion, scheduler=None):
    model.train()

    aux_data.requires_grad = False
    ...
    for ix, (tr_x, tr_reg, tr_cls) in enumerate(train_loader):
        ...
        # Foward Propagation
        pred_cls, feature = model(tr_x)
        aux_cls, aux_feature  = model(aux_data)

        loss_center = cri_center(pred_cls, tr_cls, aux_cls)
        loss_tr = cri_cls(pred_cls, tr_cls) + loss_center * option['alpha']
...
def train_out(model, optimizer, epoch, train_loader, option, criterion, scheduler=None):
    model.train()
    ...
    for ix, (tr_x, tr_cls) in enumerate(train_loader):
        # GPU Implementation
        if option['gpu'] == True:
            tr_x = tr_x.cuda()
            tr_cls = tr_cls.cuda().view(-1)
...
def test_out(model, epoch, test_loader, option):
    model.eval()
    acc_te = 0.
    ...
    with torch.no_grad():
        for ix, (te_x, te_cls) in enumerate(test_loader):
            # GPU Implementation
            if option['gpu'] == True:
                te_x = te_x.cuda()
                te_cls = te_cls.cuda().view(-1)
